...
public class Selection {
    ...
    public static <V> BlockManager select(final BlockManager blocks, final SparseBitSet selected) {
    ...
    public static <V> BlockManager select(final BlockManager blocks, final SparseBitSet rows, final SparseBitSet cols) {
        final List<List<V>> data = new LinkedList<List<V>>();
        for (int c = cols.nextSetBit(0); c >= 0; c = cols.nextSetBit(c + 1)) {
            final List<V> column = new ArrayList<V>(rows.cardinality());
